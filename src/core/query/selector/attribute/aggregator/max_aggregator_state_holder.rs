...
//! StateHolder implementation for MaxAttributeAggregatorExecutor
...
impl StateHolder for MaxAggregatorStateHolder {
    ...
    fn deserialize_state(&self, snapshot: &StateSnapshot) -> Result<(), StateError> {
        ...
        let state_data: MaxAggregatorStateData =
                ...
                StateOperation::Insert { value: new_val, .. } => {
                    ...
                    if let Some(new_v) = new_value {
                        if let Some(current_v) = *value {
                            *value = Some(current_v.max(new_v));
                        } else {
                            *value = Some(new_v);
                        }
                    }
    ...
    fn component_metadata(&self) -> StateMetadata {
        let mut metadata = StateMetadata::new(
            ...
            "MaxAttributeAggregatorExecutor".to_string(),
        );
        metadata.access_pattern = self.access_pattern();
        metadata.size_estimation = self.estimate_size();
        metadata
            .custom_metadata
            .insert("aggregator_type".to_string(), "max".to_string());
        metadata
            .custom_metadata
        ...
        if let Some(max_value) = self.get_max_value() {
            metadata
                .custom_metadata
                .insert("current_max".to_string(), max_value.to_string());
        } else {
            metadata
                .custom_metadata
                .insert("current_max".to_string(), "None".to_string());
        }
        metadata
    }
...
mod tests {
    ...
    fn test_metadata() {
        ...
        assert_eq!(
            metadata.custom_metadata.get("current_max").unwrap(),
            "456.78"
        );
        ...
        assert_eq!(
            empty_metadata.custom_metadata.get("current_max").unwrap(),
            "None"
        );
    }
}
